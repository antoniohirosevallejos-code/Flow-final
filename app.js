(() => {
  ...
  function clearBackground(t, energy) {
    ...
    } else if (visual.background === "lines") {
      drawLinesBackground(t, p, energy, intensity);
    } else {
  ...
  function drawNebulaBackground(t, p, energy, intensity) {
    const w = canvas.width;
    const h = canvas.height;
    const base = Math.min(w, h);
    ...
    gradient.addColorStop(0, p.bg[2]);
    gradient.addColorStop(.45, p.bg[0]);
    ...
    const clouds = [
      [0.22, 0.28, p.colors[2], 0.34],
      [0.76, 0.40, p.colors[1], 0.30],
      [0.46, 0.78, p.colors[0], 0.24]
    ];

    clouds.forEach((c, i) => {
      const x =
        w * c[0] +
        Math.sin(t * 0.00018 + i * 2.1) * w * 0.035;

      const y =
        h * c[1] +
        Math.cos(t * 0.00015 + i) * h * 0.025;

      const r =
        base * c[3] *
        (0.95 + energy * 0.05 * intensity);
      ...
      g.addColorStop(
        0,
        hexToRgba(c[2], 0.18 + energy * 0.06)
      );
      g.addColorStop(
        .42,
        hexToRgba(c[2], 0.055)
    ...
    // Very subtle drifting star field; deliberately sparse so it
    // doesn't compete with the visualizer or text.
    for (let i = 0; i < 42; i++) {
      const x = seeded(i * 7.31) * w;
      const y = seeded(i * 11.17 + 3) * h;
      const pulse =
        0.35 +
        0.65 *
        Math.abs(Math.sin(t * 0.001 + i * 1.7));
