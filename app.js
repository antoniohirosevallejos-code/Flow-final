(() => {
  ...
  function clearBackground(t, energy) {
    ...
    } else if (visual.background === "rays") {
      drawNebulaBackground(t, p, energy, intensity);
    } else {
  ...
  function drawRayBackground(t, p, energy, intensity) {
    const w = canvas.width;
    const h = canvas.height;
    ...
    baseGradient.addColorStop(0, p.bg[1]);
    baseGradient.addColorStop(.52, p.bg[2]);
    baseGradient.addColorStop(1, "#020206");
    ...
    // Wide, slow-moving light beams. They are deliberately sparse and
    // low-contrast so the text and reactive visualizer remain dominant.
    const beamCount = 7;
    for (let i = 0; i < beamCount; i++) {
      const angle =
        -Math.PI * 0.86 +
        i * (Math.PI * 0.28) +
        Math.sin(t * 0.00012 + i * 1.7) * 0.035;

      const spread =
        (0.045 + (i % 3) * 0.012) *
        (0.88 + energy * 0.05 * intensity);

      const length = Math.max(w, h) * 1.35;
      const grad = ctx.createLinearGradient(
        0, 0, Math.cos(angle) * length, Math.sin(angle) * length
      );

      const alpha = 0.055 + energy * 0.018 * intensity;
      const color = p.colors[i % 3];

      grad.addColorStop(0, hexToRgba(color, alpha * 1.8));
      grad.addColorStop(.42, hexToRgba(color, alpha));
    ...
    // Small central glow gives the beams a source without becoming a blob.
    const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, base * 0.42);
    glow.addColorStop(0, hexToRgba(p.colors[0], 0.10 + energy * 0.035));
    glow.addColorStop(.34, hexToRgba(p.colors[1], 0.035));
