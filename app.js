(() => {
  ...
  function planets(energy, t) {
    ...
    // Keep the system away from the text zone when the text is above/centered.
    const cy =
      visual.position === "top"
        ? h * .67
        : h * .50;
    ...
    const systems = [
      { rx: base * .13, ry: base * .065, speed: .00038, phase: .2, size: .010, color: p.colors[0] },
      { rx: base * .22, ry: base * .105, speed: -.00025, phase: 2.1, size: .016, color: p.colors[1] },
      { rx: base * .31, ry: base * .145, speed: .00016, phase: 4.4, size: .023, color: p.colors[2] }
    ];

    ctx.save();

    // Very subtle orbital rings.
    ctx.lineWidth = Math.max(1, base * .0011);
    systems.forEach((s, i) => {
      ctx.strokeStyle = hexToRgba(s.color, .12);
      ctx.beginPath();
      ctx.ellipse(
        cx,
        cy,
    ...
    // Central star: small, stable and mildly music-reactive.
    const starR = base * (.020 + energy * .008 * intensity);
    const star = ctx.createRadialGradient(cx, cy, 0, cx, cy, starR * 3.8);
    star.addColorStop(0, hexToRgba(p.colors[0], .95));
    star.addColorStop(.25, hexToRgba(p.colors[1], .42));
    star.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = star;
    ctx.beginPath();
    ctx.arc(cx, cy, starR * 3.8, 0, Math.PI * 2);
    ctx.fill();
    ...
    systems.forEach((s, i) => {
      ...
      const rot = -.10 + i * .08;
      const lx = Math.cos(angle) * s.rx;
      const ly = Math.sin(angle) * s.ry;
      const x = cx + lx * Math.cos(rot) - ly * Math.sin(rot);
      const y = cy + lx * Math.sin(rot) + ly * Math.cos(rot);
      const r = base * s.size * (1 + energy * .10 * intensity);
      ...
      planet.addColorStop(0, "rgba(255,255,255,.88)");
      planet.addColorStop(.20, hexToRgba(s.color, .98));
      planet.addColorStop(1, hexToRgba(s.color, .22));

      ctx.fillStyle = planet;
      ctx.shadowColor = s.color;
      ctx.shadowBlur = Math.max(2, base * .004);
      ctx.beginPath();
    ...
    // Sparse stars with deterministic positions, no particle-heavy system.
    for (let i = 0; i < 30; i++) {
      const sx = seeded(i * 17.13 + 2.7) * w;
      const sy = seeded(i * 9.71 + 8.4) * h;
      const twinkle = .30 + .70 * Math.abs(Math.sin(t * .001 + i * 1.9));
      const sr = Math.max(1, base * .0011);
      ctx.fillStyle = hexToRgba(p.colors[i % p.colors.length], .13 * twinkle);
      ctx.beginPath();
      ctx.arc(sx, sy, sr, 0, Math.PI * 2);
      ctx.fill();
    }

    // Occasional deterministic shooting star; cheap enough for preview/export.
    const cycle = 11;
    const phase = (t * .001 / cycle) % 1;
    if (phase > .80 && phase < .93) {
      const q = (phase - .80) / .13;
      const sx = w * (.10 + q * .95);
      const sy = h * (.18 + q * .28);
      const tail = base * .065;
      const g = ctx.createLinearGradient(sx - tail, sy - tail * .35, sx, sy);
      g.addColorStop(0, "rgba(255,255,255,0)");
      g.addColorStop(1, hexToRgba(p.colors[0], .72));
      ctx.strokeStyle = g;
      ctx.lineWidth = Math.max(1, base * .002);
      ctx.beginPath();
      ctx.moveTo(sx - tail, sy - tail * .35);
      ctx.lineTo(sx, sy);
      ctx.stroke();
    }
